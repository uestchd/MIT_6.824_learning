...
func (rf *Raft) sendingHeartBeat() {
	state := leader
	for state == leader {
		rf.sendAppendEntriesToAll(Entry{0,0})
...
func (rf *Raft) sendAppendEntriesToEach(serverid int, e Entry, args *AppendEntriesArgs, reply *AppendEntriesreply) bool{
	rf.mu.Lock()
